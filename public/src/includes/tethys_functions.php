...
function urlEncodeArr($params, $skip = null) {
	...
	if (isset($skip)) {
		$skip = (is_array($skip)) ? $skip : array($skip);
	} else {
		$skip = array();
	}
...
function outputSharingLinks($include = null) {
	...
	foreach ($include as $i) {	
		switch($i) {
			...
			break;
			case 'stumbleupon':
				$ret .= '<span class="st_stumbleupon_hcount" displayText="StumbleUpon" st_url="'.$trackUrl.'"></span>';
			break;
			case 'email':
...
function encryptPassword($passwd, &$userSalt = null) {
	if ($userSalt == '') {
		// generate new user salt
		mt_srand(microtime(true)*100000+memory_get_usage(true));
		$userSalt = substr(md5(uniqid(mt_rand(), true)),0,22);
	}
	
	// if blowfish is available, use this for encryption
	if (CRYPT_BLOWFISH==1) {
		$salt = '$2a$07$'.$userSalt.'$';
		$p = crypt($passwd, $salt);
	} // fallback to standard md5 encryption
	else {
		$p = md5($userSalt.$passwd);
	}
